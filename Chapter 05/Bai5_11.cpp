...
#define MAX 50
/*
	Tinh tong hai ma tran vuong
*/
void InpMat(float mat[][MAX], int &n);
void OutMat(float mat[][MAX], int n);
void SumTwoSquareMatrix(float mat1[][MAX], float mat2[][MAX], int n1, int n2);
...
int main() {
	...
	SumTwoSquareMatrix(mat1, mat2, n1, n2);
	return 0;
}
...
void SumTwoSquareMatrix(float mat1[][MAX], float mat2[][MAX], int n1, int n2) {
	float tong[MAX][MAX];
	if(n1 != n2) {
		printf("\nHai ma tran khong tuong thich de cong");
		exit(0);
	}
	for(int i=0; i<n1; i++) 		//So dong
		for(int j=0; j<n1; j++) 		//So cot
			tong[i][j] = mat1[i][j] + mat2[i][j];
	printf("\n===Tong hai ma tran vuong===\n");
	OutMat(tong, n1);
}
