...
#define MAX 50
/*
	Ham xuat cac so nguyen to trong ma tran so nguyen theo thu tu tang dan
*/
...
bool CheckSNT(int n) {
	...
	//Xuat cac so nguyen to tang dan
void SNTTangDan(int mat[][MAX], int m, int n) {
	int arr[m*n], viTri=-1;
	for(int i=0; i<m; i++)
		for(int j=0; j<n; j++)
			if(CheckSNT(mat[i][j])==true)
				arr[++viTri]=mat[i][j];
	printf("\nCac gia tri chan tang dan dan: ");
	TangDan(arr, viTri+1);
