...
#define MAX 50
/*
	Tim gia tri chan xuat hien dau tien trong ma tran so nguyen 
*/
void InpMat(int mat[][MAX], int &m, int &n);
void OutMat(int mat[][MAX], int m, int n);
void SearchSoChanDauTien(int mat[][MAX], int m, int n);
...
void SearchSoChanDauTien(int mat[][MAX], int m, int n) {
	for(int i=0; i<m; i++) 		//So dong
		for(int j=0; j<n; j++) 	//So cot
			if(mat[i][j]%2==0) {
				printf("\nGia tri chan dau tien la %d", mat[i][j]);
				return;
			}
