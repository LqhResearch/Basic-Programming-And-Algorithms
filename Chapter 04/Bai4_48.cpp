...
#define MAX 50
/*
	Liet ke chi so cac dong chua toan gia tri chan trong ma tran so nguyen
*/
...
void OutMat(int mat[][MAX], int m, int n) {
	...
	//Kiem tra toan so chan cua dong
bool CheckToanSoChan(int mat[][MAX], int m, int n, int row) {
	for(int j=0; j<n; j++) 	//So cot
		if(mat[row][j]%2 != 0)
			return false;
	return true;
...
void Listed(int mat[][MAX], int m, int n) {
	printf("\nChi so toan so chan: ");
	for(int i=0; i<m; i++) 		//So dong
		if(CheckToanSoChan(mat, m, n, i) == true)
			printf("%5d", i);
}
