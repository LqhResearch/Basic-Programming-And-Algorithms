...
#define MAX 50
/*
	Liet ke chi so cac dong chua gia tri chan trong ma tran so nguyen
*/
...
void OutMat(int mat[][MAX], int m, int n) {
	...
	//Kiem tra gia tri chan cua dong
bool CheckSoChan(int mat[][MAX], int m, int n, int row) {
	for(int j=0; j<n; j++) 	//So cot
		if(mat[row][j]%2==0)
			return true;
	return false;
	...
	//Liet ke chi so cac dong chua gia tri chan trong ma tran so nguyen
void Listed(int mat[][MAX], int m, int n) {
	printf("\nChi so dog chua gia tri chan: ");
	for(int i=0; i<m; i++) 		//So dong
		if(CheckSoChan(mat, m, n, i) == true)
			printf("%5d", i);
}
